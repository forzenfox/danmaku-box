...
### 从 Releases / Actions 获取扩展（无需本地构建）

推送 `v*` 标签（如 `v0.1.0`）或 main 分支更新后，GitHub Actions 会自动校验、构建并发布扩展包。两种下载方式任选：

**方式一（推荐）：GitHub Releases 下载 zip**

1. 打开仓库 **Releases** 页 → 找到对应版本（如 `v0.1.0`）
2. 在 Assets 区下载 `danmaku-box-vX.Y.Z.zip`（zip 根目录即 `manifest.json`）
3. 解压后加载：
