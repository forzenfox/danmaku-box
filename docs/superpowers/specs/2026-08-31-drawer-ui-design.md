...
* 状态：**已归档**（已被 [2026-08-31-drawer-ui-v2-design.md](2026-08-31-drawer-ui-v2-design.md) 取代，v2 已于 2026-08-31 实现并提交；仅作历史参考，不再作为实施依据）
...
| 容器   | `position: fixed` 抽屉面板 + Shadow DOM `mode: 'open'`（样式/事件隔离保留；open 允许 CDP/AX 自动化走查穿透访问把手与 iframe，隔离样式而非隔离访问） |
