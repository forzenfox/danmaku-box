...
| FR-07 | 官方收藏一键迁移 | 首次使用空库时，面板空态提供「一键导入斗鱼官方收藏」；已登录拉取官方云端收藏并可选目标分组导入（合并去重）；未登录提示先登录；官方无收藏时给出提示；仅读取官方云端数据，零上传 | P1 |
...
- **官方收藏导入边界**：仅迁移官方云端收藏（不含 DouyuEx 等第三方插件本地扩展库）；只读拉取用户自己的账号数据并写入本地，不反向写入官方云端，零上传。
- **官方收藏导入异常**：未登录提示「请先登录斗鱼」；官方无收藏提示「官方暂无收藏弹幕」；接口不可用提示「获取官方收藏失败，请稍后重试」；非斗鱼直播间页面提示在斗鱼直播间使用。
