...
2. **弹幕库面板层**：以插件图标为总入口，是收藏内容的管理中枢。面板内部为「左列分组导航 + 右侧弹幕列表」的双栏结构，顶部全局工具栏（搜索、导入、导出、新建、批量管理）、底部常驻存储用量区。面板打开时恢复上次选中的分组（PRD FR-05 设置对象 `last_selected_group`）。
